...
async function save(detail,ready){if(!detail?.id)return;const now=Date.now(),list=read(),old=list.find(x=>x.id===detail.id),item={...(old||{}),...detail,title:titleOf(detail),createdAt:old?.createdAt||now,updatedAt:now,status:ready?'ready':'draft'};delete item.blob;write([item,...list.filter(x=>x.id!==detail.id)]);if(ready&&detail.blob)await putVideo(detail.id,detail.blob);else if(!ready)await delVideo(detail.id);render()}
