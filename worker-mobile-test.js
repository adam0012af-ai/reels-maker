...
async function inject(response){
  ...
  if(!html.includes("mobile-auto-ui.css?v=8"))html=html.replace("</head>",'<link rel="stylesheet" href="mobile-auto-ui.css?v=8"></head>');
  if(!html.includes("mobile-auto-ui.js?v=8"))html=html.replace("</body>",'<script src="mobile-auto-ui.js?v=8"></script></body>');
  const h=new Headers(response.headers);h.delete("content-length");h.set("content-type","text/html; charset=utf-8");h.set("cache-control","no-store, no-cache, must-revalidate");h.set("x-rm-mobile-auto","test-v8");
