...
(()=>{
  ...
  async function fetchStockScene(query){const q=encodeURIComponent(query.slice(0,80));for(const url of [`/api/pexels?query=${q}&orientation=portrait&per_page=15`,`/api/pixabay?query=${q}&orientation=vertical&per_page=15`]){try{const r=await fetch(url);if(!r.ok)continue;const data=await r.json();if(Array.isArray(data?.videos)){for(const v of data.videos){const files=(v.video_files||[]).filter(x=>x.link&&x.height>x.width);const f=files.find(x=>x.height>=1080)||files.find(x=>x.height>=720)||files[0];if(f?.link)return{type:"video",url:f.link,query,source:"stock"}}}if(Array.isArray(data?.hits)){for(const h of data.hits){const u=h.videos?.medium?.url||h.videos?.small?.url||h.webformatURL;if(u)return{type:u.includes(".mp4")?"video":"image",url:u,query,source:"stock"}}}}catch{}}return null}
  async function searchMedia(topic,script,platform,tone,duration){let prompts=[];try{prompts=cleanQueries(await gen("visuals",topic,duration,platform,tone,{script}))}catch{}if(!prompts.length)prompts=["person thinking alone window","anxious person overthinking at home","friends discussing emotions together"];
    state.visualQueries=prompts;const items=[];for(let i=0;i<prompts.length;i++){status(`4/5 البحث عن المشهد ${i+1} المطابق للسكربت...`);let scene=await fetchStockScene(prompts[i]);if(!scene){status(`4/5 توليد بديل للمشهد ${i+1} عبر Cloudflare AI...`);scene=await generateAiScene(prompts[i])}if(scene)items.push(scene)}state.media=items;renderMedia()}
  function renderMedia(){const host=$("rmaMedia");host.innerHTML="";if(!state.media.length){host.innerHTML='<div class="rma-out">لم يتم العثور على مشاهد مناسبة للمحتوى.</div>';return}state.media.forEach((it,i)=>{const w=document.createElement("div");w.className="rma-media-item";const el=document.createElement(it.type==="video"?"video":"img");el.src=it.url;if(it.type==="video"){el.muted=true;el.loop=true;el.playsInline=true;el.autoplay=true}const b=document.createElement("span");b.textContent=`مشهد ${i+1}${it.source==="ai"?" · AI":" · Pexels"}`;w.append(el,b);host.appendChild(w)})}
  ...
  async function sendToEditor(){const btn=$("rmaToEditor");btn.disabled=true;try{status("جاري إرسال الفيديو والصوت للمحرر...");const video=state.media.find(x=>x.type==="video");if(!video)throw new Error("المشاهد الحالية صور فقط؛ لا يوجد فيديو جاهز للإرسال تلقائيًا.");const vi=$("videoUpload");if(!vi)throw new Error("محرر الفيديو غير جاهز");putFile(vi,await fetchVideo(video));if(state.voiceBlob){const ai=$("audioUpload");if(ai){const ext=state.voiceBlob.type.includes("wav")?"wav":"mp3";putFile(ai,new File([state.voiceBlob],`auto-voice.${ext}`,{type:state.voiceBlob.type||"audio/wav"}))}}close();document.querySelector('.tab[data-tab="video"]')?.click()}catch(e){status("خطأ: "+e.message)}finally{btn.disabled=false}}
  async function run(){const topic=$("rmaTopic").value.trim();if(!topic)return status("اكتب موضوع الفيديو أولاً");const d=Number($("rmaDuration").value)||45,platform=$("rmaPlatform").value,tone=$("rmaTone").value,btn=$("rmaRun"),editor=$("rmaToEditor");btn.disabled=true;editor.disabled=true;state.objectUrls.forEach(u=>URL.revokeObjectURL(u));state.objectUrls=[];state.voiceBlob=null;state.media=[];state.visualQueries=[];setText("rmaIdea","—");setText("rmaScript","—");setText("rmaVoice","—");setText("rmaCaption","—");$("rmaMedia").innerHTML='<div class="rma-out">—</div>';
    try{status("1/5 إنشاء الفكرة...");state.idea=await gen("idea",topic,d,platform,tone);setText("rmaIdea",state.idea);status("2/5 كتابة السكربت...");state.script=await gen("script",topic,d,platform,tone);setText("rmaScript",state.script);status("3/5 إنشاء الصوت...");try{await makeVoice(state.script,tone)}catch(e){setText("rmaVoice","تعذر إنشاء الصوت: "+e.message)}status("4/5 اختيار مشاهد مطابقة للسكربت...");await searchMedia(topic,state.script,platform,tone,d);status("5/5 إنشاء الكابشن...");state.caption=await gen("caption",topic,d,platform,tone);setText("rmaCaption",state.caption);editor.disabled=!state.media.some(x=>x.type==="video");status("اكتمل المحتوى. Pexels هو المصدر الأول للمشاهد، وCloudflare AI يعمل كبديل فقط عند عدم وجود نتيجة مناسبة.")}catch(e){status("خطأ: "+e.message)}finally{btn.disabled=false}}
