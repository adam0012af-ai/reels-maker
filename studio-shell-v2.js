...
(() => {
  ...
  function buildTemplatesPanel() {
    ...
    panel.innerHTML = `
      <div class="rm-template-hero">
        ...
        <p>القالب يفتح أدوات المشروع الحقيقية ويضبط نقطة البداية فقط، بدون فصل المشروع عن المحرر.</p>
      </div>
  ...
  function buildTimeline() {
    ...
    let zoom = 1;
    $("rmTimelinePlus").addEventListener("click", () => { zoom = Math.min(2, zoom + .25); $("rmTrackStack").style.transformOrigin = "left top"; $("rmTrackStack").style.setProperty("--timeline-zoom", String(zoom)); });
    $("rmTimelineMinus").addEventListener("click", () => { zoom = Math.max(.75, zoom - .25); $("rmTrackStack").style.setProperty("--timeline-zoom", String(zoom)); });
