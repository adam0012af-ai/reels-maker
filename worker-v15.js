import baseWorker from "./worker-v15-base.js";
...
function promptFor(body){
  ...
  const strict=/arab/i.test(language)?"اكتب بالعربية الطبيعية فقط. ممنوع الكلمات الإنجليزية أو الترجمات أو عناوين مثل Hook وBeat وCTA. لا تضف أي تعليمات تصوير أو نصوص تظهر على الشاشة.":"";
...
async function generateAutoContent(request,env){
  ...
  const body=await safeJson(request),prompt=promptFor(body);let last="";
  for(const model of AUTO_TEXT_MODELS){
    try{
      ...
      const text=extractText(result);if(text)return json({text,task:body.task||"idea",model});last=`${model}: no text`;
