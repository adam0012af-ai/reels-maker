...
function injectStableBoot(html){
  const head=`<style id="rmStableBootStyle">
  ...
  if(!html.includes('id="rmStableBootStyle"'))html=html.replace("<head>",`<head>${head}`);
...
function injectStableAssets(html){
  if(!html.includes("mobile-stable-v10.css"))html=html.replace("</head>",'<link rel="stylesheet" href="mobile-stable-v10.css?v=1"></head>');
  if(!html.includes("route-native-v10.js"))html=html.replace("</body>",'<script src="route-native-v10.js?v=1"></script></body>');
  return html;
}
...
async function patchCoreScript(response){
  ...
  headers.set("x-rm-mobile-runtime","v10");
...
async function injectV15(response){
  ...
  headers.set("x-rm-runtime","stable-v10");
