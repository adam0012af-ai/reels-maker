...
const state={projectId:'',story:'',scenes:[],sceneUrls:[],finalUrl:'',busy:false};
...
function newId(){return globalThis.crypto?.randomUUID?.()||`visual-${Date.now()}-${Math.random().toString(36).slice(2,8)}`}
function revokeUrl(u){if(u)try{URL.revokeObjectURL(u)}catch{}}
function resetMedia(){state.sceneUrls.forEach(revokeUrl);state.sceneUrls=[];state.scenes=[];revokeUrl(state.finalUrl);state.finalUrl='';const v=$('finalVideo');if(v){v.pause?.();v.removeAttribute('src');v.load?.()}$('finalWrap')?.classList.add('hidden')}
...
function projectDetail(blob=null){return {id:state.projectId||newId(),idea:$('storyIdea')?.value||'',story:state.story,duration:Number($('storyDuration')?.value||30),tone:$('storyTone')?.value||'natural',style:$('visualStyle')?.value||'cinematic',sceneCount:state.scenes.length,scenePrompts:state.scenes.map(s=>s.prompt),format:'short',blob}}
function emitDraft(){if(!state.projectId)state.projectId=newId();window.dispatchEvent(new CustomEvent('reels-visual-project-draft',{detail:projectDetail()}))}
function emitReady(blob){if(!state.projectId)state.projectId=newId();window.dispatchEvent(new CustomEvent('reels-visual-project-ready',{detail:projectDetail(blob)}))}

function saveDraft(){try{localStorage.setItem(DRAFT_KEY,JSON.stringify({projectId:state.projectId,idea:$('storyIdea')?.value||'',duration:$('storyDuration')?.value||'30',tone:$('storyTone')?.value||'natural',count:$('sceneCount')?.value||'auto',style:$('visualStyle')?.value||'cinematic',story:state.story,scenePrompts:state.scenes.map(s=>s.prompt)}))}catch{}}
function restoreDraft(){try{const d=JSON.parse(localStorage.getItem(DRAFT_KEY)||'null');if(!d)return;state.projectId=d.projectId||'';if($('storyIdea'))$('storyIdea').value=d.idea||'';if($('storyDuration'))$('storyDuration').value=d.duration||'30';if($('storyTone'))$('storyTone').value=d.tone||'natural';if($('sceneCount'))$('sceneCount').value=d.count||'auto';if($('visualStyle'))$('visualStyle').value=d.style||'cinematic';if(d.story){state.story=d.story;$('storyText').textContent=d.story;$('storyText').classList.remove('empty');set('storyStatus','تم استرجاع آخر مسودة. اضغط إنشاء القصة والمشاهد لاستكمال الصور.')}}catch{}}
...
async function renderVideo(){if(state.busy)return;setBusy(true);pipe('render');set('storyStatus','جاري تجهيز الفيديو الصامت…');try{const blob=await renderSilentVideo();revokeUrl(state.finalUrl);state.finalUrl=URL.createObjectURL(blob);$('finalVideo').src=state.finalUrl;$('downloadVideo').href=state.finalUrl;$('finalWrap').classList.remove('hidden');finishPipes();emitReady(blob);set('storyStatus',`اكتمل الفيديو · ${$('storyDuration')?.value||30} ثانية · ${state.scenes.length} مشاهد.`);$('finalWrap').scrollIntoView({behavior:'smooth',block:'center'})}catch(e){set('storyStatus','خطأ التصدير: '+(e?.message||e))}finally{setBusy(false)}}

function restoreSelect(id,value){const el=$(id);if(el&&[...el.options].some(o=>o.value===String(value)))el.value=String(value)}
function openSavedProject({project,blob}={}){if(!project)return;resetMedia();state.projectId=project.id||newId();state.story=project.story||'';if($('storyIdea'))$('storyIdea').value=project.idea||'';restoreSelect('storyDuration',project.duration||30);restoreSelect('storyTone',project.tone||'natural');restoreSelect('visualStyle',project.style||'cinematic');restoreSelect('sceneCount',project.sceneCount||'auto');$('storyText').textContent=state.story||'لا يوجد نص محفوظ.';$('storyText').classList.toggle('empty',!state.story);state.scenes=(project.scenePrompts||[]).map(prompt=>({prompt,blob:null,url:''}));renderScenes();$('renderVisualVideoBtn')?.classList.add('hidden');if(blob){state.finalUrl=URL.createObjectURL(blob);$('finalVideo').src=state.finalUrl;$('downloadVideo').href=state.finalUrl;$('finalWrap').classList.remove('hidden')}saveDraft();set('storyStatus',blob?'تم فتح المشروع والفيديو المحفوظ. لإعادة الصور اضغط إنشاء القصة والمشاهد.':'تم فتح المسودة. اضغط إنشاء القصة والمشاهد لاستكمال الصور.')}
...
export function initVisualStoryStudio(){installVisualCss();simplifyNavigation();refreshHome();buildStoryPage();window.addEventListener('reels-open-visual-project',e=>openSavedProject(e.detail));window.addEventListener('beforeunload',saveDraft)}
